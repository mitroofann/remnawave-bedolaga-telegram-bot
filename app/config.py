...
class Settings(BaseSettings):
    ...
    # При истечении подписки снимать у юзера все сквады в панели (ветка A), чтобы RemnaWave
    # показывала корректное «подписка истекла», а не мёртвые хосты. Пер-тариф можно вместо
    # этого выдать free-сквад и держать доступ N дней (ветка B, поля тарифа expire_free_squads
    # + expire_free_days). Сквады возвращаются при продлении/смене тарифа. См. expire_squad_service.
    CUSTOM_EXPIRE_CLEAR_SQUADS_ENABLED: bool = True
