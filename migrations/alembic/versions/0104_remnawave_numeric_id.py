...
# [Форк] 2026-09: возвращён апстримовый down='0103' (при мерже upstream 4.7.0
# форк-стек целиком перецеплен в ХВОСТ за 0118 через 0104_landing_trial, поэтому
# эта ревизия снова стоит на своём исходном месте). Миграция полностью
# идемпотентна (каждый шаг под inspector-guard), что позволяет повторные прогоны
# при перестройках цепочки. См. [[alembic-merge-revision-collision]].
revision = '0104'
down_revision = '0103'
branch_labels = None
depends_on = None
