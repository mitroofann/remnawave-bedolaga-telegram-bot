...
# [Форк] down_revision перецеплен '0103' → '0108': прод форка уже застемплен на
# конец форк-стека (0108), а апстрим завёл эту ревизию как down='0103'. Оставь её
# «в прошлом» — и `alembic upgrade head` счёл бы её пройденной и НЕ создал числовые
# колонки remnawave_id, после чего bot v4.0.0 падал бы на 'expected number,
# received NaN'. Ставим в ХВОСТ, чтобы прод применил её вперёд. Миграция полностью
# идемпотентна (каждый шаг под inspector-guard), поэтому позиция в хвосте безопасна
# и на свежей БД. См. [[alembic-merge-revision-collision]].
revision = '0104'
down_revision = '0108'
branch_labels = None
depends_on = None
