...
async def cmd_broadcast(message: Message, bot: Bot) -> None:
    ...
    [Форк] Рассылка отключена — работает как любое другое сообщение: отвечаем заглушкой.
    База данных не используется (весь код рассылки сохранён ниже, но не вызывается).
    """
    await _send_stub(message)

    # Код ниже не выполняется: связь с БД не нужна, нужна только заглушка.
